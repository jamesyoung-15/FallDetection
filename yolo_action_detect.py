...
def inference(num_people, keypts, frame, conf_threshold=0.5):
    # for each person
...
def video_inference(vid_source="/dev/video0", vid_width=640, vid_height=640, show_frame=True, manual_move=False, interval=0):    
    ...
    while True:
        ...
        if elapsed_time >= interval:
            ...
            for result in results:
                ...
                # if keypts detected
                if num_pts !=0:
                    inference(num_people, keypts, frame, conf_threshold=0.5)

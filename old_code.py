...
def stream_inference(vid_source="/dev/video0", vid_width=640, vid_height=640, show_frame=True, manual_move=False, interval=0, 
    ...
    model = YOLO("models/yolo-weights/yolov8n-pose.pt")
...
def video_inference(vid_source="./test-data/videos/fall-1.mp4", vid_width=480, vid_height=480, 
    ...
    # load pretrained model
    model = YOLO("models/yolo-weights/yolov8s-pose.pt")
    ...
    if save_video == True:
        ...
        video_output = cv2.VideoWriter('demo-3.avi',  fourcc, 30, (int(cap.get(3)),int(cap.get(4)))) 
    ...
    while True:
        ...
        # after num_frames_elapsed frames, run inference, to use time interval use: if elapsed_time >= interval:
        if num_frames_elapsed >= interval:
            # update timer
            # prev_time = curr_time
            num_frames_elapsed = 0
